...
class Skin_Grid_Havsula extends Skin_Base {
    ...
    $this->add_control(
    	...
    	$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::HEADING,
				'condition' => [
					'skin_grid_havsula_show_category!' => '',
				],
			]
		);
    ...
    $this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .elementor-event__read-more:hover' => 'background-color: {{VALUE}}',
				],
        		'condition' => [
					'skin_grid_havsula_show_read_more!' => '',
				],
        ...
        <?php if( '' !== $this->parent->get_instance_value_skin( 'show_thumbnail' ) ) { ?>
          <div class="elementor-event__thumbnail">
            ...
            <?php
              ...
              if( '' !== $this->parent->get_instance_value_skin( 'show_category' ) ){
                the_terms( get_the_ID(), 'tribe_events_cat', '<div class="elementor-event__category">' , ', ', '</div>' );
              }

              if( '' !== $this->parent->get_instance_value_skin( 'show_read_more' ) ) {
                echo '<a class="elementor-event__read-more" href="' . get_the_permalink() . '">' . alone_addons_get_icon_svg( 'chevron-right', 16 ) . '</a>';
              }
            ?>
  				</div>
  			<?php } ?>
          ...
          <?php if( '' !== $this->parent->get_instance_value_skin( 'show_meta' ) ) { ?>
	          <ul class="elementor-event__meta">
	              ...
	              if( $venue_id ) {
	                echo '<li class="address">' . alone_addons_get_icon_svg( 'location', 14 );
	                  $this->parent->event_addres( $venue_id );
	                echo '</li>';
	              }

                echo tribe_events_event_schedule_details( get_the_ID(), '<li class="date">' . alone_addons_get_icon_svg( 'clock', 14 ), '</li>' );
	    				?>
	          </ul>
					<?php } ?>
				</div>
