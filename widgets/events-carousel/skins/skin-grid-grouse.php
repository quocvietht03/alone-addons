    ...
    $this->add_control(
    	...
    	$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .elementor-event__meta span' => 'color: {{VALUE}}',
				],
        		'condition' => [
					'skin_grid_grouse_show_meta!' => '',
				],
          ...
          <?php if( '' !== $this->parent->get_instance_value_skin( 'show_meta' ) ) { ?>
            ...
            <ul class="elementor-event__meta">
              <?php
                ...
                echo '<li>' . tribe_get_start_date( get_the_ID(), false, $time_format ) . '</li>';

								$organizer_id = get_post_meta( get_the_ID(), '_EventOrganizerID', true);
	              if( $organizer_id ) {
									echo '<li>
												 <span>' . esc_html__('By ', 'alone-addons') . '</span>' .
