<?php
namespace AloneAddons\Widgets\Sermone_Carousel;
...
class Be_Sermone_Carousel extends Widget_Base {

	public function get_name() {
		return 'be-sermone-carousel';
	}

	public function get_title() {
		return __( 'Be Sermone Carousel', 'alone-addons' );
	}
  ...
  protected function get_supported_books_taxonomies() {
	...
	protected function register_query_section_controls() {
		...
		$this->add_control(
			'preacher',
			[
				'label' => __( 'Preacher', 'alone-addons' ),
				'type' => Controls_Manager::SELECT2,
	...
	protected function swiper_data() {
		...
		if( '' !== $settings['pagination'] ) {
			if( '' !== $settings['_skin'] ) {
				$el_class = '.sermone-carousel-dots--' . $settings['_skin'];
			} else {
				$el_class = '.sermone-carousel-dots--default';
			}
		...
		<div class="swiper-slide">
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'elementor-sermon' ); ?>>
				<?php if( '' !== $settings['show_thumbnail'] ) { ?>
		          ...
		          <?php echo alone_addons_get_icon_svg( 'search', 16 ); ?>
		        </a>
					</div>
				<?php } ?>
				...
				<div class="elementor-sermon__content">
					...
					<ul class="elementor-sermon__meta">
						<li><?php echo __( 'On ', 'alone-addons' ) . sermone_date_format( '', sermone_get_field( 'sermon_date_preached', get_the_ID() ) ) ?></li>
	          <li><?php echo get_the_term_list( get_the_ID(), 'sermone_preacher', __( 'by ', 'alone-addons' ), ', ', '.' ) ?></li>
	        </ul>
