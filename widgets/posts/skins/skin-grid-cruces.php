<?php
namespace AloneAddons\Widgets\Posts\Skins;
...
class Skin_Grid_Cruces extends Skin_Base {
    ...
    $this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'condition' => [
					'skin_grid_cruces_show_read_more!' => '',
				],
			]
		);
        ...
        <div class="elementor-post__content">
          ...
          <?php if( '' !== $this->parent->get_instance_value_skin('show_meta') ) { ?>
  	          ...
  	          <li>
  	            <?php
  								echo '<a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">'.
  									alone_addons_get_icon_svg('user', 14) .
              ...
              <?php
                if ( has_category() ) {
                  echo '<li>';
                    echo alone_addons_get_icon_svg('tags', 14);
                    the_category( ', ' );
                  echo '</li>';
                }
              ?>
          ...
          <?php
						if( '' !== $this->parent->get_instance_value_skin('show_read_more' ) ){
							echo '<a class="elementor-post__read-more" href="' . get_the_permalink() . '">' . $this->parent->get_instance_value_skin('read_more_text') . '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" width="24" height="24" x="0" y="0" viewBox="0 0 612 612" style="enable-background:new 0 0 512 512" xml:space="preserve" fill="currentColor" class=""><g><path d="M535.5 0h-459C34.253 0 0 34.253 0 76.5v459C0 577.747 34.253 612 76.5 612h459c42.247 0 76.5-34.253 76.5-76.5v-459C612 34.253 577.747 0 535.5 0zm38.25 535.5c0 21.133-17.117 38.25-38.25 38.25h-459c-21.133 0-38.25-17.117-38.25-38.25v-459c0-21.114 17.117-38.25 38.25-38.25h459c21.133 0 38.25 17.117 38.25 38.25v459zM450.126 289.189l-108.19-108.19c-7.478-7.478-19.584-7.478-27.042 0-7.479 7.478-7.479 19.584 0 27.043l78.813 78.833H191.25c-10.557 0-19.125 8.568-19.125 19.125s8.568 19.125 19.125 19.125h202.457l-78.813 78.814c-7.479 7.478-7.479 19.584 0 27.042 7.478 7.479 19.584 7.479 27.042 0l108.19-108.189c4.59-4.59 6.005-10.863 4.973-16.811 1.032-5.91-.402-12.202-4.973-16.792z"></path></g></svg></a>';
						}
					?>
        </div>
			</article>
