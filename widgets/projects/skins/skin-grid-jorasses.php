<?php
namespace AloneAddons\Widgets\Projects\Skins;
...
class Skin_Grid_Jorasses extends Skin_Base {

	protected function _register_controls_actions() {
		add_action( 'elementor/element/be-projects/section_layout/before_section_end', [ $this, 'register_layout_controls' ] );
		add_action( 'elementor/element/be-projects/section_design_layout/before_section_end', [ $this, 'registerd_design_layout_controls' ] );
    	add_action( 'elementor/element/be-projects/section_design_layout/after_section_end', [ $this, 'register_design_box_section_controls' ] );
		add_action( 'elementor/element/be-projects/section_design_layout/after_section_end', [ $this, 'register_design_image_section_controls' ] );
    	add_action( 'elementor/element/be-projects/section_design_layout/after_section_end', [ $this, 'register_design_content_section_controls' ] );

	}

	public function get_id() {
		return 'skin-grid-jorasses';
	}


	public function get_title() {
		return __( 'Grid Jorasses', 'alone-addons' );
	}
    ...
    $this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::NUMBER,
				'default' => apply_filters( 'jorasses_excerpt_length', 15 ),
				'condition' => [
		...
		$this->add_control(
			...
			[
				...
				'default' => apply_filters( 'jorasses_excerpt_more', '' ),
				'condition' => [
					'skin_grid_jorasses_show_excerpt!' => '',
				],
			]
	...
	public function register_design_image_section_controls( Widget_Base $widget ) {
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .elementor-project__thumbnail' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
    ...
    $this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .elementor-project__title a:hover' => 'color: {{VALUE}};',
				],
				'condition' => [
					'skin_grid_jorasses_show_title!' => '',
				],
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::HEADING,
				'condition' => [
					'skin_grid_jorasses_show_category!' => '',
				],
			]
		);
	...
	protected function render_post() {
		...
		<article id="post-<?php the_ID();  ?>" <?php post_class( 'elementor-project' ); ?>>
          ...
          if( '' !== $this->parent->get_instance_value_skin('show_category') ) {
						the_terms( 'project_category', '<div class="elementor-project__category">' . alone_addons_get_icon_svg('tags', 14) , ', ', '</div>' );
					}
				?>
			</div>
		</article>
